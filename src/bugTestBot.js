...
const token = 'NzY3NzQ1NDY1OTQ0NjM3NDQw.X42Yrw.JKkNbAqHJWxrPhGbQm_SNG7Kb8c';
let d = new Date();
imgur.setClientId('eb0de232a3f47ae');
const repoid = '0';
...
const parseLink = require(`./util/parseLink.js`);
...
const arrayEquals = require(`./util/arrayEquals.js`);
	...
	let commandLUT = {
		...
		"submit": async function() {
			...
			const embed = new Discord.RichEmbed()
				...
				.setColor(0xFF00FF);
			const sent = await message.guild.channels.get('787992855142072321').send(embed);
			selfDeleteReply(message, `your report has been submitted!`);
			delay('4s', () => {message.delete()});
			await sent.react('787970450570608640');
		},
		...
		"edit": async function() {
			...
			if(!/^\d+$/.test(rid)) return selfDeleteReply(message, `input \`${rid}\` is not a snowflake`);
			try  {
				const target = await message.guild.channels.get('787992855142072321').fetchMessage(rid);
				delay('3s', () => {message.delete()});
		...
		"canrepro": async function() {
			...
			try {
				...
				delay('3s', () => {message.delete()});
				const embed = new Discord.RichEmbed(target.embeds[0]);
				const splitMessage = message.content.split("-w ")
				if (!embed.fields[4]) {
				embed.addField('Can Reproduce','<a:yes2:787970450570608640>' + `${message.author.username}#${message.author.discriminator} : `+ `${splitMessage[1]}`)
				config1.set('crokay', '<a:yes2:787970450570608640>' + `${message.author.username}#${message.author.discriminator} : ` + `${splitMessage[1]}`)
				config1.set('crokay1', config1.get('crokay'))
				} else {
					config1.delete('crokay1')
					config1.set('crokay1', config1.get('crokay'))
					config1.delete('crokay')
					config1.set('crokay', config1.get('crokay1') + '\n<a:yes2:787970450570608640>' + `${message.author.username}#${message.author.discriminator} : ` + `${splitMessage[1]}`)
					embed.fields[4].value = `${config1.get('crokay')}`
				}
				await target.edit(embed);
				selfDeleteReply(message, `updated report at ${rid}`);
			} catch(e) {
		...
		"cantrepro": async function(){
			...
			try {
				...
				const splitMessage = message.content.split("-w ")
				if (!embed.fields[5]) {
				embed.addField('Cant Reproduce','<a:no2:787984280327815168>' + `${message.author.username}#${message.author.discriminator} : `+ `${splitMessage[1]}`)
				config1.delete('cnrokay1')
				config1.delete('cnrokay')
				config1.set('cnrokay', '<a:no2:787984280327815168>' + `${message.author.username}#${message.author.discriminator} : ` + `${splitMessage[1]}`)
				config1.set('cnrokay1', config1.get('cnrokay'))
				} else {
					config1.delete('cnrokay1')
					config1.set('cnrokay1', config1.get('cnrokay'))
					config1.delete('cnrokay')
					config1.set('cnrokay', config1.get('cnrokay1') + '\n<a:no2:787984280327815168>' + `${message.author.username}#${message.author.discriminator} : ` + `${splitMessage[1]}`)
					embed.fields[5].value = `${config1.get('cnrokay')}`
				}
				await target.edit(embed);
				selfDeleteReply(message, `updated report at ${rid}`);
			} catch(e) {
		...
		"dlink": async function() { // broken due to imgur website design update
			const escapedLinks = [];
